...
public class ChatInput extends JPanel {
    public ChatInput(Runnable sync, Function<String, Void> sendMessage) {
        ...
        area.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_ENTER && e.getModifiersEx() == 0) {
                    ...
                    sendMessage.apply(area.getText());
                    area.setText("");
                }
            }
        });
        ...
        JButton syncButton = new JButton(
            new FlatSVGIcon(
                ChatInput.class.getResource("/icons/sync.svg")
            ).derive(18, 18).setColorFilter(new FlatSVGIcon.ColorFilter(
                color -> UIManager.getColor("primary")
            ))
        );
        syncButton.setToolTipText("Press to manually sync with remote server");
        syncButton.putClientProperty(FlatClientProperties.BUTTON_TYPE, FlatClientProperties.BUTTON_TYPE_TOOLBAR_BUTTON);
        syncButton.addActionListener(e -> sync.run());
        this.add(syncButton);
